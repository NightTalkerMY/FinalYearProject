...
class HandDetector:
    ...
    def findHands(self, img, draw=True):
        ...
        """
        imgRGB = cv.cvtColor(img, cv.COLOR_BGR2RGB) ## open cv see as BGR, so we need to convert it to RGB
        self.results = self.hands.process(imgRGB) ## sends image to mediapipe network and got the result (21 landmark)
        allHands = []
        h, w, c = img.shape
        ...
        if self.results.multi_hand_landmarks:
            for handLms in self.results.multi_hand_landmarks:
                ...
                for id, lm in enumerate(handLms.landmark):
                    lmCoords_3D.append([lm.x, lm.y, lm.z]) ## getting 3d coordinate
                    px, py = int(lm.x * w), int(lm.y * h) ## this is to append the 2d into screen
                    lmCoords_2D.append([px, py, 0.0]) ## save 2d coordinate
                    xList.append(px) 
                    yList.append(py)
