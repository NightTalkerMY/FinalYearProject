...
def modelCheckpoint(learn, learn_directory):
    ...
    # 1. Define the folder relative to where the script is running
    # .resolve() turns "../checkpoints" into the full path "D:\FYP\Gesture_System\checkpoints"
    dest_folder = Path("../checkpoints").resolve()
    
    # 2. Create the folder if it does not exist
    dest_folder.mkdir(parents=True, exist_ok=True)
    
    # 3. Create the full file path
    save_path = dest_folder / f"{_stem}.pkl"
    
    # 4. Load the best weights
    learn = learn.load(file=deets.e_model_tag, with_opt=True)
    
    # 5. Export using the ABSOLUTE path. 
    # This forces fastai to ignore its own 'models' directory logic.
    learn.export(fname=save_path)
    
    print(f"Learner pkl and pt checkpoints created successfully at: {save_path}")
# -----------------------------------------------
...
# [FIX 1/3]: Corrected TunerImages to use B/W logic (ignore Y)
class tunerImages(fastuple):
    @classmethod
    def create(cls, preds): 
        # MODIFIED: Accepts only 'preds' (prediction vector), ignores 'y'
        # Uses B/W logic (White bars) to match Inference
        img_size = 560
        l_band = img_size // preds.shape[-1]

        imgs = [np.zeros([img_size, img_size, 3], dtype=np.uint8) for vo in preds]
        stt_bands = [(np.argmax(vo)*l_band) for vo in preds]

        for _idx, (_img, _stt_band) in enumerate(zip(imgs, stt_bands)):
            # Always paint the predicted band WHITE [255, 255, 255]
...
# [FIX 2/3]: Corrected Model to stop asking for labels
class end2endTunerModel(Module):
    def __init__(self, archMultiVOs, archTuner, dls_vocab, tuner_img_sz=224, debug=False):
        ...
        self.multiVOsBody = create_body(archMultiVOs(pretrained=True), cut=None)
        self.multiVOsHead = create_head(nf=num_features_model(self.multiVOsBody), n_out=len(dls_vocab))
        self.tunerBody = create_body(archTuner(pretrained=True), cut=None)
    ...
    def forward(self, Xy, y=None):
        # We must accept Xy to be compatible with the Dataloader structure
        if y is not None: X, y = Xy, y  
        else:             X, y = Xy      
        ...
        # 1. Get predictions from the base models (MultiVOs)
        ...
        # 2. Generate Tuner Images (FIXED: No longer passes 'y')
        tnr_X = self.batch_tuner_images(mvo_preds)
        
        # 3. Get predictions from the Tuner
        ...
        # Return all features so the Loss Function can use them
        return [*mvo_ftrs, tnr_ftrs]

    def batch_tuner_images(self, preds):
        # FIXED: Removed 'targs' argument entirely to match Inference logic
        batch = []
        preds = preds.detach().cpu().numpy()

        for _idx, _X in enumerate(preds):
            # Pass only the prediction vector to the image creator
            img = tunerImages.create(preds=_X)
...
# [FIX 3/3]: Fixed Loss Wrapper (Version 2 with correct Init and Math)
class e2eTunerLossWrapper(Module):
    def __init__(self, nOutputs, baseLoss=CrossEntropyLossFlat):
        super().__init__()
        self.baseLoss = baseLoss()
        self.log_vars = nn.Parameter(torch.zeros(nOutputs))

    def forward(self, preds: list, targs):
        mvos_losses = 0
        
        # Multi-output (auxiliary) losses
        for idx, p in enumerate(preds[:-1]):
            raw = self.baseLoss(p, targs)
            precision = torch.exp(-self.log_vars[idx])
            loss = precision * raw + self.log_vars[idx]
            mvos_losses += loss

        # Final predictor loss
        raw = self.baseLoss(preds[-1], targs)
        precision = torch.exp(-self.log_vars[-1])
        tnr_loss = precision * raw + self.log_vars[-1]

        return mvos_losses + tnr_loss

    def decodes(self, preds):
        return [p.argmax(dim=-1) for p in preds]

    def accuracyTuner(self, preds, targs):
        return accuracy(preds[-1], targs)
# -----------------------------------------------
