        ...
        if hand_data:
            ...
            if gate.anchor_pos is not None:
                if state == "IDLE" or state == "WARMUP":
                    ...
                    cv.putText(img, state, (cx-60, cy-70), cv.FONT_HERSHEY_SIMPLEX, 0.5, (200,200,200), 1)
...
if __name__ == "__main__":
    ...
    # Launch subprocesses (Hidden Logic)
    ...
    # REMOVED: vispy_output_gui = subprocess.Popen([python_exe, "vispyOutputGUI.py"])

    try:
        live_stream_hgr(nD="3d")
        print("\nINFO: Exiting main.py normally. ", end="")
