...
#         # === FIX START ===
#         # The data comes in as ((Image_View_1, Image_View_2...), Internal_Label)
#         # We check if X[0] is a tuple/list (the images). If so, we strip the label (X[1]).
#         if isinstance(X, (list, tuple)) and len(X) == 2 and isinstance(X[0], (list, tuple)):
#             X = X[0]
            
#         mvo_ftrs = []
#         for xi in X:
#             # DEBUG: Print the shape to see exactly what we are dealing with
#             # if self.debug: print(f"DEBUG: xi type={type(xi)}, shape={xi.shape if isinstance(xi, torch.Tensor) else 'N/A'}")

#             # 1. Unpack if it is a list/tuple
#             while isinstance(xi, (list, tuple)):
#                 xi = xi[0]
            
#             # 2. Ensure it is 4D (Batch, Channel, Height, Width) for Conv2d
#             # If it's 3D (C, H, W), add a batch dimension
#             if isinstance(xi, torch.Tensor) and xi.dim() == 3:
#                 xi = xi.unsqueeze(0)

#             # 3. Pass to model
#             mvo_ftrs.append(self.multiVOsBody(xi))
#         # === FIX END ===
...
class end2endTunerModel(Module):
    def __init__(self, archMultiVOs, archTuner, tuner_img_sz=224, debug=False):
        ...
        # Note: We keep cut=None and create_head standard for inference
        self.multiVOsBody = create_body(arch=archMultiVOs, cut=None)
        self.multiVOsHead = create_head(nf=num_features_model(self.multiVOsBody), n_out=args.n_classes)
        self.tunerBody = create_body(arch=archTuner, cut=None)
        self.tunerHead = create_head(nf=num_features_model(self.tunerBody), n_out=args.n_classes)
    ...
    def forward(self, X):
        ...
        # === FIX START: Robust Tuple Handling ===
        # This ensures the model works whether X is a Tensor or a (Tensor,) tuple
        if isinstance(X, (list, tuple)) and len(X) >= 1:
             # If it's a list like [Tensor, Label], take the Tensor
             # If it's a list like [Tensor], take the Tensor
            X = X[0]
            
        # Handle the case where X is a list of tensors (one per view)
        mvo_ftrs = []
        
        # If X is still a list/tuple at this point, iterate through it
        if isinstance(X, (list, tuple)):
            iterator = X
        else:
            iterator = [X] # Wrap single tensor in list to iterate

        for xi in iterator:
             # 1. Unpack if it is nested
            while isinstance(xi, (list, tuple)):
                xi = xi[0]
            
            # 2. Ensure it is 4D (Batch, Channel, Height, Width)
            if isinstance(xi, torch.Tensor) and xi.dim() == 3:
                xi = xi.unsqueeze(0)
        ...
        # Note: In inference file, we use 'tunerImagesBW' class name
        tnr_X = self.batch_tuner_images(mvo_preds)
    ...
    def batch_tuner_images(self, preds):
        ...
        for _idx, _X in enumerate(preds):
            # Using the Inference Class Name: tunerImagesBW
            img = tunerImagesBW.create(X=_X) 
            img = compose_tfms(img, self.item_tfms)
            if self.debug and (_idx == 0): print(f">> {img.shape= }") ; img.show() # type:ignore
...
class e2eTunerLossWrapper(Module):
    def __init__(self, nOutputs, baseLoss=CrossEntropyLossFlat):
        super().__init__()  # <--- Critical for avoiding crashes
        self.baseLoss = baseLoss()
