...
# e2eET Skeleton Based HGR Using Data-Level Fusion
# Dynamic Hand Gestures Classification: Backend Renderer
# -----------------------------------------------
...
def create_sequence_png():
    ...
    cfg.gs_tag.replace(f"{cfg.gs_images_directory}/{cfg.gs_tag.name}")
    hgrLogger(f">HGR: @{cfg.gs_tag.name} sequence visualized.", log=cfg.hgr_log)
    _set_view(viz_VO)

# [INIT VISPY SCENE OBJECTS]___________________________________________________
# NOTE: show=False makes this run "backend" without popping up windows
canvas = scene.SceneCanvas(
    keys=None, # Disabled interactivity
    title="e2eET HGR: Backend Fusion",
    app="PyQt5",
    always_on_top=False,
    vsync=False,
    bgcolor="black",
...
class loadConfigArguments:
    def __init__(self) -> None:
        ...
        self.sz_canvas = config.setdefault("sz_canvas", 960)
        self.n_lims = config.setdefault("n_processes", 1)
        self.fps = np.inf # Go as fast as possible
...
def main():
    global data_files, l_data_files
    if l_data_files:
        cfg.init_gesture_sequence(data_files.pop())
    else:
        data_files = deque(sorted(cfg.gs_data_directory.iterdir(), reverse=True))
...
if __name__ == "__main__":
    ...
    _set_view(viz_VO)
    _make_colored_fingers()
    print("INFO: Initialized <dataLevelFusion.py> (Backend Mode)...")
